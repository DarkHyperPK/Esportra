...
describe('tournamentLifecycle', () => {
  ...
  it('opens check-in windowMinutes before start and closes at start', () => {
    const start = new Date('2026-06-14T20:45:00.000Z');
    const window = resolveCheckInWindow({
      startDate: start,
      settings: { checkInWindowMinutes: 30 },
    });

    expect(window.opensAt?.toISOString()).toBe('2026-06-14T20:15:00.000Z');
    expect(window.closesAt?.toISOString()).toBe(start.toISOString());

    const beforeOpen = new Date('2026-06-14T20:10:00.000Z');
    expect(beforeOpen.getTime()).toBeLessThan(window.opensAt!.getTime());
    expect(beforeOpen.getTime() - window.opensAt!.getTime()).toBe(-5 * 60_000);
  });
});
