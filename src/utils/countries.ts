...
export const detectUserCountry = async (): Promise<string | null> => {
    // List of free geolocation APIs that are known to be CORS-friendly
    const apis = [
        { url: 'https://api.country.is', key: 'country' },
        { url: 'https://get.geojs.io/v1/ip/geo.json', key: 'country_code' },
    ...
    for (const api of apis) {
        try {
            ...
            // Use 'no-cache' to avoid getting old location data
            ...
            if (response.ok) {
                ...
                if (code && typeof code === 'string' && code.length === 2) {
                    console.log(`Successfully detected country using ${api.url}: ${code}`);
                    return code.toUpperCase();
                }
            }
        } catch (error) {
            console.warn(`Geolocation fallback failed for ${api.url}:`, error);
        }
    }
