 ...
 * Keep tools/button-color-classname.cjs in sync for CI checks.
 */
...
/** Strip Tailwind variant prefixes (hover:, sm:, dark:, etc.). */
...
function isNonColorTextUtility(suffix: string): boolean {
...
export function tokenIsColorUtility(token: string): boolean {
...
/** True when className contains Tailwind color utilities that belong on variants. */
export function classNameHasColorUtilities(className: string | undefined): boolean {
