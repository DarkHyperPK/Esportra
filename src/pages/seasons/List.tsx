import { Link } from 'react-router-dom';
import Footer from '@/components/Footer';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { useSeasons } from '@/hooks/useSeasons';
import { ArrowRight, Trophy, Workflow } from 'lucide-react';

const SeasonsList = () => {
  const { data: seasons, isLoading, error, refetch } = useSeasons();

  return (
    <div className="min-h-screen bg-[#050505] text-white">
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-10">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-rose-400">Season hub</p>
          <h1 className="mt-3 text-4xl font-black tracking-tight">Connected qualifier trees</h1>
          <p className="mt-3 max-w-2xl text-sm text-zinc-400">
            Explore active and published seasons, follow how qualifiers feed finals, and track who has already earned
            downstream spots.
          </p>
        </div>

        {isLoading && (
          <div className="rounded-3xl border border-white/10 bg-black/20 p-10 text-center text-zinc-400">
            Loading seasons...
          </div>
        )}

        {error && (
          <div className="rounded-3xl border border-red-500/20 bg-red-500/10 p-8">
            <p className="font-semibold text-red-100">Could not load seasons.</p>
            <p className="mt-2 text-sm text-red-200/80">{error instanceof Error ? error.message : 'Unknown error'}</p>
            <Button onClick={() => refetch()} className="mt-4 bg-white/10 text-white hover:bg-white/20">
              Retry
            </Button>
          </div>
        )}

        {!isLoading && !error && seasons && seasons.length === 0 && (
          <div className="rounded-[32px] border border-dashed border-white/10 bg-black/20 p-12 text-center">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-3xl bg-rose-500/10 text-rose-300">
              <Workflow className="h-7 w-7" />
            </div>
            <h2 className="mt-5 text-2xl font-semibold">No public seasons yet</h2>
            <p className="mx-auto mt-3 max-w-xl text-sm text-zinc-400">
              Organizers have not published any season trees yet. Check back after the next season rollout.
            </p>
          </div>
        )}

        {!isLoading && !error && seasons && seasons.length > 0 && (
          <div className="grid gap-5 lg:grid-cols-2">
            {seasons.map((season) => (
              <div key={season.id} className="rounded-[28px] border border-white/10 bg-black/30 p-6 backdrop-blur-xl">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="text-2xl font-bold">{season.name}</h2>
                      <Badge className="bg-rose-500/10 text-rose-300 hover:bg-rose-500/10">{season.status}</Badge>
                    </div>
                    <p className="mt-2 text-sm text-zinc-400">{season.description || 'No description yet.'}</p>
                  </div>

                  <div className="rounded-2xl bg-white/5 p-3 text-zinc-300">
                    <Trophy className="h-5 w-5" />
                  </div>
                </div>

                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-zinc-500">Game</p>
                    <p className="mt-1 font-semibold text-white">{season.game}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-zinc-500">Mode</p>
                    <p className="mt-1 font-semibold text-white">{season.participantMode}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-zinc-500">Start</p>
                    <p className="mt-1 font-semibold text-white">{season.startDate || 'TBA'}</p>
                  </div>
                </div>

                <div className="mt-6">
                  <Button asChild className="bg-rose-500 text-white hover:bg-rose-600">
                    <Link to={`/seasons/${season.id}`}>
                      Open season
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Link>
                  </Button>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>

      <Footer />
    </div>
  );
};

export default SeasonsList;

