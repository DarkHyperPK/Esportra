...
const TeamsPage = () => {
  ...
  const isCaptain = currentTeam?.created_by === user?.id;
  
  // Debug logging
  console.log('=== TEAM DEBUG ===');
  console.log('Current user ID:', user?.id);
  console.log('Team created_by:', currentTeam?.created_by);
  ...
  const fetchTeamRegistrations = async () => {
    ...
    try {
      // First, try to get registrations without the join to see if the table exists
      const { data: registrations, error: regError } = await supabase
        .from('tournament_registrations')
        .select('*')
        .eq('team_id', currentTeam.id);

      console.log('Basic registrations query result:', { registrations, regError });

      if (regError) {
        console.error('Error with basic registrations query:', regError);
        ...
        console.log('Alternative registrations query result:', { altRegistrations, altRegError });
      ...
      if (registrations && registrations.length > 0) {
        ...
        if (tournamentIds.length > 0) {
          const { data: tournaments, error: tournamentError } = await supabase
            .from('tournaments')
            .select('id, name, start_date, game, prize_pool')
            .in('id', tournamentIds);
      ...
      console.log('Set team registrations:', registrations || []);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-esports-dark flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <div className="text-esports-primary text-xl font-semibold mb-2">Organizer Mode</div>
          ...
          <div className="text-xs text-gray-500">
            As an organizer, you can create tournaments but not teams. Switch roles to access team features.
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-esports-dark text-white">
        <div className="container-professional spacing-section">
          <div className="text-center-professional mb-16">
            <h1 className="text-6xl font-bold mb-6 text-esports-primary">
              No Team Found
            </h1>
            <p className="text-xl text-esports-secondary max-w-2xl mx-auto">
              You need to create a team to participate in tournaments.
            </p>
          </div>
          
          {/* Single team creation card */}
          <div className="max-w-2xl mx-auto">
            <div className="card-esports spacing-card text-center-professional">
              <div className="mb-8">
                <div className="w-24 h-24 bg-esports-blue rounded-xl flex-center mx-auto mb-6">
                  <Users className="w-12 h-12 text-white" />
                </div>
                <h2 className="text-3xl font-bold text-esports-primary mb-4">Create Your Esports Team</h2>
                <p className="text-lg text-esports-secondary mb-6">
                  Form your own esports team and compete across multiple games. You'll be the team captain and can invite other players to join.
                </p>
              </div>
              <div className="flex-center">
                <Button 
                  size="lg" 
                  className="btn-esports-blue spacing-button rounded-lg font-semibold text-lg px-12 py-4"
      ...
      <div className="container-professional spacing-section">
        ...
        <div className="card-esports spacing-card mb-8">
          ...
          {(() => {
            ...
            let captainMember = currentTeam.members?.find((member: any) => member.user_id === currentTeam.created_by);
            ...
            return captainMember ? (
              ...
              <div className="text-center text-white/60 py-8">
                ...
                <p className="text-sm">User ID: {user?.id}, Team created_by: {currentTeam?.created_by}</p>
              </div>
            );
          })()}
        </div>
        ...
        <div className="card-esports spacing-card mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {currentTeam.members?.filter((member: any) => {
              // Only filter out the captain (team creator) from members list
              // Show all other members including the current user if they're not the captain
              const isCaptain = member.user_id === currentTeam.created_by;
              ...
              console.log(`Current user ID: ${user?.id}`);
              console.log(`Is captain? ${isCaptain}`);
              console.log(`Should show in members? ${!isCaptain}`);
              console.log(`========================`);
              
              return !isCaptain;
            }).map((member: any) => (
              <div key={member.id} className={`rounded-lg p-4 border ${
                member.user_id === currentTeam.created_by 
                  ? 'bg-red-500/20 border-red-400' // Highlight captain if they appear here
          ...
          {currentTeam.members?.filter((member: any) => member.user_id !== currentTeam.created_by).length === 0 && (
            <div className="text-center text-white/60 py-8">
