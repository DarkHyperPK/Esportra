 ...
 return (
  <div className="space-y-6">
   <div className="flex items-center gap-4">
    ...
    {selectedId && (
     <button
      ...
      className="flex items-center gap-1.5 px-4 py-2.5 bg-rose-600 hover:bg-rose-500 text-sm text-white font-medium transition-colors"
     >
   ...
   {selectedId && !isLoading && selectedSponsor && (
    <>
     {/* Sponsor info bar */}
     <div className="flex items-center gap-3 px-4 py-3 bg-zinc-900/50 border border-zinc-800">
      ...
      <span className="text-[10px] font-mono uppercase px-2 py-0.5 bg-zinc-800  text-zinc-400">{displayTier(selectedSponsor.tier)}</span>
     ...
     {Object.entries(tournamentGroups).map(([tid, group]) => (
      <div key={tid} className="bg-zinc-900/50 border border-zinc-800 p-5">
     ...
     {placements.length === 0 && (
      <div className="border border-dashed border-zinc-800 py-12 text-center text-sm text-zinc-600">
     ...
     {isSuperAdmin && (
      <div className="bg-zinc-900/50 border border-zinc-800 p-5 space-y-5">
       <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
         <BarChart2 className="w-4 h-4 text-rose-400" />
         <h4 className="text-sm font-bold text-white">Analytics</h4>
        </div>
        <div className="flex bg-zinc-800 border border-zinc-700 p-0.5 gap-0.5">
         {DAY_OPTIONS.map(d => (
          <button
           key={d}
           onClick={() => setDays(d)}
           className={`px-3 py-1  text-xs font-bold transition-colors ${days === d ? 'bg-zinc-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
           {d}d
          </button>
         ))}
       ...
       {summary && (
        <div className="grid grid-cols-3 gap-3">
         {[
          ...
          <div key={label} className="bg-zinc-950 border border-zinc-800 p-4">
           <p className="text-[10px] font-mono uppercase tracking-wider text-zinc-500 mb-1">{label}</p>
           <div className="flex items-end gap-1.5">
            <span className="text-lg font-bold text-white">{value}</span>
            {trend !== undefined && trend !== 0 && (
             <span className={`flex items-center gap-0.5 text-[10px] font-mono mb-0.5 ${trend > 0 ? 'text-emerald-300' : 'text-red-400'}`}>
