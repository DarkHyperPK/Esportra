...
export function ResolvePlacementDialog({ placement, tournaments, onClose }: Props) {
 ...
 return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-labelledby="resolve-title">
  <div className="w-full max-w-md border border-zinc-700 bg-zinc-900 p-5">
   ...
   <div className="mt-5 space-y-4">
    <label className="block text-xs text-zinc-400">Zone<select value={zone} onChange={event => { setZone(event.target.value as PlacementZone); setSlotNumber(1); }} className="mt-1 w-full  border border-zinc-700 bg-zinc-950 p-2 text-white">{Object.entries(ZONE_META).map(([value, valueMeta]) => <option key={value} value={value}>{valueMeta.label}</option>)}</select></label>
    {!meta.isGlobal && <label className="block text-xs text-zinc-400">Tournament<select value={tournamentId} onChange={event => setTournamentId(event.target.value)} className="mt-1 w-full  border border-zinc-700 bg-zinc-950 p-2 text-white"><option value="">Select tournament</option>{tournaments.map(tournament => <option key={tournament.id} value={tournament.id}>{tournament.name}</option>)}</select></label>}
    <label className="block text-xs text-zinc-400">Slot<select value={slotNumber} onChange={event => setSlotNumber(Number(event.target.value))} className="mt-1 w-full  border border-zinc-700 bg-zinc-950 p-2 text-white">{Array.from({ length: meta.maxSlots }, (_, index) => <option key={index + 1} value={index + 1}>Slot {index + 1}</option>)}</select></label>
   </div>
   {mutation.isError && <p className="mt-3 text-xs text-red-400">{mutation.error.message}</p>}
   <div className="mt-5 flex justify-end gap-2"><button onClick={onClose} className=" px-4 py-2 text-sm text-zinc-400">Cancel</button><button onClick={submit} disabled={mutation.isPending || (!meta.isGlobal && !tournamentId)} className=" bg-amber-500 px-4 py-2 text-sm font-semibold text-black disabled:opacity-40">{mutation.isPending ? 'Resolving…' : 'Resolve'}</button></div>
  </div>
 </div>;
}