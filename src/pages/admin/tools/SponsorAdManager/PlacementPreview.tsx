...
export const PlacementPreview: React.FC<Props> = ({ open, onClose, placements, tournamentName }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-zinc-950 border border-zinc-800 rounded-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-zinc-800 sticky top-0 bg-zinc-950 z-10">
          ...
          <button onClick={onClose} className="p-1.5 hover:bg-zinc-800 rounded"><X className="w-5 h-5 text-zinc-400" /></button>
        </div>
          ...
          <div className="border border-zinc-800 rounded-lg overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-12 gap-6">
              ...
              <div className="col-span-9 space-y-6">
                {/* Content placeholder */}
                <div className="bg-zinc-900 border border-zinc-800 rounded p-4 h-24 flex items-center justify-center">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-6 py-4 border-t border-zinc-800">
                    {logos.length > 0 ? logos.slice(0, 4).map(p => (
                      ...
                      <div key={i} className="h-8 w-16 bg-zinc-800 rounded animate-pulse" />
                    ))}
                  </div>
                </div>
              </div>
            ...
            {Object.entries(ZONE_META).filter(([_, m]) => !m.isGlobal).map(([key, meta]) => {
              ...
              return (
                <span key={key} className="flex items-center gap-1.5">
                  <span className={`w-2 h-2 rounded-full ${count > 0 ? 'bg-emerald-500' : 'bg-zinc-700'}`} />
...
const PreviewSlot: React.FC<{ placement: Placement; aspect: string }> = ({ placement, aspect }) => {
  ...
  return (
    <div className={`${aspect} bg-zinc-900 border border-zinc-700 rounded overflow-hidden relative group`}>
      {image ? (
      ...
      <div className="absolute bottom-1 left-1 px-1.5 py-0.5 bg-black/60 rounded text-[8px] text-zinc-400 font-mono">
...
const EmptySlot: React.FC<{ label: string; aspect: string }> = ({ label, aspect }) => (
  <div className={`${aspect} border border-dashed border-zinc-800 rounded flex items-center justify-center`}>
