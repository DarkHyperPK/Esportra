 ...
 const form = useForm({
  defaultValues: {
   ...
   games: venue.games,
    ...
    price_range: venue.price_per_hour ? `$${venue.price_per_hour}/hr` : '$10-20/hr'
  }
 });
   ...
   <DialogContent className="sm:max-w-4xl bg-[#0a0a0c] border border-white/10 text-white overflow-y-auto overscroll-contain max-h-[90vh]" data-lenis-prevent>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 my-4">
     ...
     <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
       {images.map((img, idx) => (
        <div key={idx} className="relative aspect-video group overflow-hidden border border-white/10">
      ...
      <div className="bg-white/[0.03] border border-white/10 p-4 flex gap-3 items-start">
    ...
    <DialogFooter className="mt-6 border-t border-white/5 pt-4">
     ...
     <button
      ...
      className="relative inline-flex h-9 items-center justify-center overflow-hidden border border-white bg-white px-3 font-mono text-[11px] font-bold uppercase tracking-wider text-[#0a0a0c] transition-colors duration-300 hover:bg-rose-500 hover:text-white disabled:pointer-events-none disabled:opacity-50"
      disabled={loading}
     >
