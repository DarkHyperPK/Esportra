    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-4xl overflow-y-auto border-zinc-800 bg-[#0a0a0c] text-white" data-lenis-prevent>
        <DialogHeader>
          ...
          <DialogDescription>Super-admin view. All values remain privacy suppressed.</DialogDescription>
        </DialogHeader>
        ...
        {report.isLoading ? <State text="Loading privacy-safe audience report…" />
          : report.isError ? <State text="Audience report could not be loaded." action={() => void report.refetch()} />
          : !report.data ? null
          : report.data.status === 'empty' ? <State text="No qualifying impressions in this period." />
          : report.data.status === 'suppressed' ? <State text={`At least ${report.data.privacy.minimumAudience} unique visitors are required.`} />
          : <div className="space-y-6">
              ...
              <p className="rounded-xl border border-white/5 p-4 text-xs text-zinc-500">Segments below {report.data.privacy.minimumAudience} visitors are withheld. Unknown is coverage only and is never redistributed.</p>
            </div>}
      </DialogContent>
    </Dialog>
  );
...
const Metric = ({ label, value }: { label: string; value: string }) => <div className="rounded-xl border border-white/5 p-4"><p className="text-xs text-zinc-500">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></div>;
