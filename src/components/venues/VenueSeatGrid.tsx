  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <Monitor className={`w-4 h-4 ${style.text}`} />
      ...
      {seat.displayName && seat.status === 'occupied' && (
        <span className="inline-flex items-center gap-1 text-[11px] text-zinc-400 truncate">
          <User className="w-3 h-3 shrink-0" /> {seat.displayName}
        </span>
      )}
      {seat.expiresAt && (
        <span className="inline-flex items-center gap-1 text-[11px] text-zinc-500">
          <Clock className="w-3 h-3 shrink-0" /> {formatTimeRemaining(seat.expiresAt)}
        </span>
      )}
    </motion.div>
  );
    ...
    <section>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <h2 className="text-lg font-semibold text-white flex items-center gap-2">
          <Monitor className="w-5 h-5 text-rose-500" /> Live Stations
        </h2>
