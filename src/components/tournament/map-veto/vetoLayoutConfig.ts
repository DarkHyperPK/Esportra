...
export function getVetoLayoutConfig(mode: VetoLayoutMode, isComplete: boolean): VetoLayoutConfig {
  ...
  return {
    ...
    grid: cn(
      ...
      isModal && 'md:grid-cols-[minmax(200px,24%)_minmax(0,1fr)] md:gap-3 lg:grid-cols-[minmax(220px,22%)_minmax(0,1fr)]',
    ...
    stageGrid: cn(
      ...
      'grid-cols-1',
      isModal && 'md:grid-cols-[minmax(0,1fr)_minmax(200px,30%)] lg:grid-cols-[minmax(0,1fr)_minmax(220px,28%)]',
      isEmbedded && 'lg:grid-cols-[minmax(0,1fr)_minmax(240px,30%)]',
      isFullscreen && 'xl:grid-cols-[minmax(0,1fr)_minmax(280px,32%)]',
    ),
    sequenceScroll: cn(
      'min-h-0',
      isModal && 'max-h-[min(38vh,320px)] overflow-y-auto overscroll-contain pr-0.5 md:max-h-[min(52vh,420px)]',
    ...
    mapPool: {
      ...
      minHeight: isModal ? '6.5rem' : '7.5rem',
      gap: isModal ? 'gap-2 sm:gap-2.5' : 'gap-2.5 sm:gap-3',
      mapNameSize: isModal
