...
const MatchHistoryCard: React.FC<MatchHistoryCardProps> = ({ matchData, targetPuuid }) => {
    ...
    const [mapData, setMapData] = useState<{ displayName?: string; listViewIcon?: string } | null>(null);
    const [isExpanded, setIsExpanded] = useState(false);
    const [activeTab, setActiveTab] = useState<'scoreboard' | 'economy' | 'rounds' | 'weapons'>('scoreboard');
    const [allAgents, setAllAgents] = useState<Record<string, { displayIcon?: string }>>({});
    ...
    useEffect(() => {
        ...
        fetch('https://valorant-api.com/v1/agents?isPlayableCharacter=true')
            .then((res) => res.json())
            .then((data) => {
                const map: Record<string, { displayIcon?: string }> = {};
                data.data.forEach((agent: { uuid: string; displayIcon?: string }) => {
        ...
        fetch('https://valorant-api.com/v1/maps')
            .then((res) => res.json())
            .then((data) => {
                const foundMap = data.data.find((mapEntry: { mapUrl?: string }) => mapEntry.mapUrl === mapUri);
                setMapData(foundMap);
            });
