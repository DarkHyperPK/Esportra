-- =====================================================
-- SAFE COMPREHENSIVE SUPABASE FIX SCRIPT
-- =====================================================
-- This script fixes all the issues we've identified, checking for column existence first
...
-- 3. CHECK AND RENAME COLUMNS SAFELY
-- =====================================================

-- Check if created_by column exists in teams table and rename it
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'teams' AND column_name = 'created_by' AND table_schema = 'public') THEN
        ALTER TABLE public.teams RENAME COLUMN created_by TO owner_id;
        RAISE NOTICE 'Renamed created_by to owner_id in teams table';
    ELSE
        RAISE NOTICE 'created_by column does not exist in teams table, skipping rename';
    END IF;
END $$;

-- Check if created_by column exists in tournaments table and rename it
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'tournaments' AND column_name = 'created_by' AND table_schema = 'public') THEN
        ALTER TABLE public.tournaments RENAME COLUMN created_by TO owner_id;
        RAISE NOTICE 'Renamed created_by to owner_id in tournaments table';
    ELSE
        RAISE NOTICE 'created_by column does not exist in tournaments table, skipping rename';
    END IF;
END $$;

-- 4. UPDATE INDEXES SAFELY
-- =====================================================
...
SELECT '🎉 Safe comprehensive Supabase fix completed successfully!' as message;
SELECT '✅ RLS disabled, policies removed, columns checked/renamed, indexes updated' as status;
