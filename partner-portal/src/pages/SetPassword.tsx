...
const SetPassword = () => {
    ...
    useEffect(() => {
        // If no session, user shouldn't be here (unless flow handles it differently)
        // Usually invite link logs them in automatically via hash fragment
        supabase.auth.getSession().then(({ data: { session } }) => {
            if (!session) {
                // If checking session fails, maybe wait a bit or redirect to login?
                // Actually, let's just let them try. If update fails, we know why.
            }
        });
    }, []);
