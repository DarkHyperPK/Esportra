...
| Source | Contents |
|---|---|
| `CLAUDE.md` | Architecture, layer rules, data fetching, match-room realtime, auth roles, security (blocking), design quality, testing, git workflow, feature build order |
| `.claude/rules/*.md` | Guardrails and language rules — blocking policy |
| `.claude/skills/*/SKILL.md` | Domain skills — load the relevant skill before working in its area |
| `.claude/commands/*.md` | Workflow definitions (`feature-development`, `database-migration`, `add-language-rules`) |
| `.claude/team/`, `.claude/homunculus/`, `.claude/research/`, `.claude/enterprise/` | Supporting conventions and instincts |
