...
- If a layer is unreachable (e.g., no backend locally), say so explicitly and mark the finding conditional — never assume what that layer does.
...
- [ ] Component state: is it derived, mirrored, or duplicated? Can it drift from server truth?
- [ ] Effects: dependencies complete? Cleanup present? Can it run before data is ready?
- [ ] Async: ordering, races, stale closures, double-invocation (StrictMode), in-flight guards.
- [ ] Validation: does validation accept values the rest of the stack can't handle (and vice versa)?
- [ ] Request payload: exactly what is sent? Formats, timezones, null vs empty.
- [ ] Response handling: what the client receives and how it re-renders.
...
| Failure modes | What happens on partial failure, retry, double-click, stale data, invalid input? |
