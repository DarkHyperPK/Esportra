-- REFACTOR & FIX: Bracket Generation System
-- This script consolidates all fixes into one safe operation.
-- It ensures the schema is correct and installs the simplified logic.

BEGIN;

-- 1. SCHEMA FIX: stage_participants
-- Ensure 'team_id' exists and 'participant_id' is gone.
DO $$
BEGIN
    -- Migrate data from participant_id if it exists
    ...
    -- Drop legacy participant_id column
    ...
    -- Enforce NOT NULL on team_id
    ALTER TABLE public.stage_participants ALTER COLUMN team_id SET NOT NULL;
END $$;

-- 2. CLEANUP: Remove unused RPCs
-- We are switching to direct logic, so we don't need the separate seeding function anymore.
DROP FUNCTION IF EXISTS public.seed_stage_from_registrations(UUID);

-- 3. LOGIC UPDATE: Simplified Bracket Generation
-- Directly uses registered participants if the stage is empty.
...
BEGIN
    ...
    -- First, check if teams are already in the stage
    ...
    -- If no teams in stage, pull directly from tournament registrations
